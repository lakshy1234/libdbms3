...
const Userdata = () => {
  ...
  const [deleteUserloginId, setDeleteUserloginId] = useState(null);
  ...
  useEffect(() => {
    const fetchAllUserlogins = async () => {
      setIsLoading(true);
      setError(null);
      try {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setUserlogins(userlogins.filter((userlogin) => userlogin.id !== id));
  ...
  const handleCheckboxChange = (userloginId) => {
    setSelectedUserlogins((prevSelectedUserlogins) =>
      prevSelectedUserlogins.includes(userloginId)
        ? prevSelectedUserlogins.filter((id) => id !== userloginId)
        : [...prevSelectedUserlogins, userloginId]
    );
  };
  ...
  const handleSelectAllChange = () => {
    if (selectAll) {
      ...
      setSelectedUserlogins(userlogins.map((userlogin) => userlogin.id));
    }
    setSelectAll(!selectAll);
  };

  const openDeleteDialog = (id) => {
    setDeleteUserloginId(id);
    setDeleteDialogOpen(true);
  };

  const closeDeleteDialog = () => {
    setDeleteUserloginId(null);
      ...
      <div className="nav-search" id="nav-search">
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell className='column-title'style={{ fontSize: '15px' }}>First Name</TableCell>
                <TableCell className='column-author'style={{ fontSize: '15px' }}>Last Name</TableCell>
                <TableCell className='column-desc'style={{ fontSize: '15px' }}>Email</TableCell>
                <TableCell className='column-price'style={{ fontSize: '15px' }}>Contact No.</TableCell>
                <TableCell className='column-price'style={{ fontSize: '15px' }}>Books Borrowed</TableCell>
                <TableCell className='column-actions'style={{ fontSize: '15px' }}>Password</TableCell>
              ...
              {userlogins.map((userlogin, index) => (
                <TableRow key={userlogin.id}>
                  <TableCell padding="checkbox">
                    <input
                      type="checkbox"
                      checked={selectedUserlogins.includes(userlogin.id)}
                      onChange={() => handleCheckboxChange(userlogin.id)}
                    />
                  </TableCell>
                  <TableCell style={{ fontSize: '12px' }}>{index + 1}</TableCell>
                  <TableCell style={{ fontSize: '12px' }}>{userlogin.userid}</TableCell>
                  <TableCell style={{ fontSize: '12px' }}>{userlogin.first_name}</TableCell>
                  <TableCell style={{ fontSize: '12px' }}>{userlogin.last_name}</TableCell>
                  <TableCell style={{ fontSize: '12px' }}>{userlogin.email}</TableCell>
                  <TableCell style={{ fontSize: '12px' }}>{userlogin.phone_no}</TableCell>
                  <TableCell style={{ fontSize: '12px' }}>{userlogin.Books_Borrowed}</TableCell>
                  <TableCell style={{ fontSize: '12px' }}>{userlogin.password}</TableCell>
                  ...
                  onClick={() => openAddDialog(userlogin.id)}
                >
                </Button>
                    <Button
                      component={Link}
                      to={`/update/${userlogin.id}`}
                      size="small"
                      variant="contained"
                      className="buttonu"
                      //color="primary"
                    ...
                    <Button
                     ...
                     // style={{ width: '20px' }}
                      ...
                      onClick={() => openDeleteDialog(userlogin.id)}
                    >
                    </Button>
                    <Button>
        ...
        >
          ...
          <DialogActions>
            ...
            <Button onClick={() => handleDelete(deleteUserloginId)} color="secondary" autoFocus>
              Delete
            </Button>
          </DialogActions>
        </Dialog>
